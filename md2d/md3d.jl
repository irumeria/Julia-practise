...
function do_FCC_simulation(steps::Int)
    ...
    efcc = expandLattice(FCC_BASIC_PRIMITIVE_COORDINATE, lattice_size)
    efcc = map(e -> e |> float, efcc)

    @show size(FCC_BASIC_PRIMITIVE_COORDINATE)
    @show size(expandLattice(efcc, lattice_size))
